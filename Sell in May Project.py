...
def seasonal_diff_bar(t,p):
    ...
    plt.xlabel("Season")
    plt.ylabel("Average Percentage Difference from Annual Average")
    plt.title("\n".join(wrap(f"{yf.Ticker(t).info.get("longName")} — Seasonal Stock Price: Winter (Nov–Apr) vs Summer (May–Oct) Across {title_period(p)}.",60)))
    plt.show()
...
raw_scatter("^FTSE","25y") #FIRST VAR IS TICKER, SECOND IS PERIOD, THIS ONLY WORKS FROM 1YR BACK TO 2010
monthly_diff_bar("^FTSE","25y") #MAX 40 YEARS DUE TO LIBRARY LIMITATIONS
seasonal_diff_bar("^FTSE","25y")
>>>>>>> 30ada6d (Initial commit - project start)
